...
export type AgentProviderOptions = Record<string, any> | undefined;
...
/**
 * Formats a channel config XML tag from configured locales.
 */
export function formatChannelConfigTag(locales?: string[]): string {
    const list = locales && locales.length > 0 ? locales : ["en", "zh-tw"];
    return `<channel_config locales="${list.join(", ")}" />`;
}
