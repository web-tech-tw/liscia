...
describe("Anthropic Thinking & Provider Options Utilities", () => {
    ...
    describe("formatUserProfileTag & formatUserProfileContext", () => {
        it("should return empty tag when sender is missing or empty", () => {
            ...
            expect(formatUserProfileTag(undefined)).toBe("");
            expect(formatUserProfileTag({})).toBe("");
            expect(formatUserProfileContext("Hello world", null)).toBe("Hello world");
        });
    ...
    describe("formatChannelConfigTag", () => {
        it("should format channel config with default locales when omitted or empty", () => {
            expect(formatChannelConfigTag()).toBe("<channel_config locales=\"en, zh-tw\" />");
            expect(formatChannelConfigTag([])).toBe("<channel_config locales=\"en, zh-tw\" />");
        });

        it("should format channel config with custom multi-language locales", () => {
            expect(formatChannelConfigTag(["en", "zh-tw", "ja"])).toBe("<channel_config locales=\"en, zh-tw, ja\" />");
        });
    });
});
